...
async function handleSiweVerify(req, origin) {
    ...
    const {
        data: nrow,
        error: nerr
    } = await sb.from("siwe_nonces").select("nonce,expires_at,used").eq("address", lower).single();
...
async function handleRefresh(req, origin) {
    ...
    const {
        data: s,
        error
    } = await sb.from("auth_sessions").select("id,user_id,refresh_hash,created_at,expires_at,hard_expires_at,revoked_at").eq("refresh_hash", rth).single();
